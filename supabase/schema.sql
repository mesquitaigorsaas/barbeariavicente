-- =====================================================================
-- Barbearia Ferreira · Havaí (BH) — agendamento online (Supabase / Postgres)
-- Cole tudo no Supabase → SQL Editor → Run.
--
-- ATENÇÃO: a primeira parte APAGA os agendamentos/serviços/horários antigos
-- (estrutura anterior do projeto) e recria tudo. O acesso ao painel (tabela admins) é mantido.
--
...
-- Recomeça do zero (mantém a tabela admins = quem acessa o painel).
drop function if exists public.create_booking(integer[], timestamptz, text, text, text);
drop function if exists public.create_booking(integer[], timestamptz, integer, text, text, text);
drop function if exists public.busy_intervals(date);
drop function if exists public.get_booking(uuid);
drop table if exists bookings, blocks, services, hours, settings, professionals cascade;
...
insert into hours (weekday, closed, opens, closes) values
  -- PROVISÓRIO: o Google não informa o horário. Confirmar com o Renilton e ajustar no painel.
  (0, true,  '09:00', '19:00'),
  (1, false, '09:00', '19:00'),
  (2, false, '09:00', '19:00'),
  (3, false, '09:00', '19:00'),
...
select * from (values
  ('Corte',          'Degradê, social ou na tesoura', null::numeric, 40, 'main', 1),
  ('Barba',          'Barba feita e desenhada',       null::numeric, 30, 'main', 2),
  ('Corte e Barba',  'O combo completo',              null::numeric, 60, 'main', 3),
  ('Corte infantil', 'Para a criançada',              null::numeric, 40, 'main', 4)
) as v(name, description, price, duration_min, kind, sort)
where not exists (select 1 from services);

insert into professionals (name, sort)
select 'Renilton Ferreira', 1 where not exists (select 1 from professionals);
